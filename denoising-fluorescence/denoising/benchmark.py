...
DEFAULT_PRETRAIN_DIR = 'experiments/pretrained'
...
parser.add_argument('--data-root', default=DEFAULT_DATA_ROOT, type=str, help='dir to dataset')
parser.add_argument('--pretrain-dir', default=DEFAULT_PRETRAIN_DIR, type=str, help='dir to pre-trained model')
...
run_path = project_path(run_dir)

with open(run_path / 'args.txt') as args_file:
...
model.load_state_dict(torch.load(run_path / f'checkpoints/model_epoch{args.epochs}.pth',
    map_location='cpu'))
...
for noise_level in noise_levels:
    for image_type in image_types:
        ...
        mkdirs(project_path(test_case_dir))
        tic = time.time()
        ...
        for i in range(n_plots):
            ...
            plot_row(np.concatenate((fixed_noisy_stitched, fixed_denoised_stitched, 
                fixed_clean_stitched)), str(project_path(test_case_dir)), f'denoising{i}',
                same_range=True, plot_fn='imshow', cmap=cmap, colorbar=False)

        with open(project_path(test_dir) / "results_{}.txt".format('cpu' if args_test.no_cuda else 'gpu'), 'w') as args_file:
