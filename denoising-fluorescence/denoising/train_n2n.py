...
from paths import DEFAULT_DATA_ROOT, project_path
import os
import sys
...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--exp-dir', type=str, default=DEFAULT_EXP_DIR, help='directory to save experiments')
        ...
        # data
        self.add_argument('--data-root', type=str, default=DEFAULT_DATA_ROOT, help='directory to dataset root')
        ...
        self.add_argument('--noise-levels-train', type=int, nargs='+', default=[1])
        self.add_argument('--noise-levels-test', type=int, nargs='+', default=[1])
        ...
        self.add_argument('--num-workers', type=int, default=0, help='DataLoader worker processes')
        # training
    ...
    def parse(self):
        ...
        args.run_dir = str(Path(args.exp_dir) / args.exp_name / date /
        ...
        args.ckpt_dir = str(Path(args.run_dir) / 'checkpoints')

        if not args.post:
            mkdirs([project_path(args.run_dir), project_path(args.ckpt_dir)])
        ...
        if not args.post:
            with open(project_path(args.run_dir) / "args.txt", 'w') as args_file:
...
args.train_dir = str(Path(args.run_dir) / "training")
args.pred_dir = str(Path(args.train_dir) / "predictions")
mkdirs([project_path(args.train_dir), project_path(args.pred_dir)])
if args.net == 'unet':
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.ckpt_freq == 0:
            torch.save(model.state_dict(), project_path(args.ckpt_dir) / "model_epoch{}.pth".format(epoch))
        ...
        with torch.no_grad():
            ...
            if epoch % args.plot_epochs == 0:
                ...
                save_samples(str(project_path(args.pred_dir)), samples, epoch, 'test', epoch=True, cmap=args.cmap)
                # fixed test
                fixed_denoised = model(fixed_test_noisy)
                samples = torch.cat((fixed_test_noisy[:4].cpu(), 
                    fixed_denoised[:4].cpu(), fixed_test_clean[:4], 
                    fixed_denoised[:4].cpu() - fixed_test_clean[:4]))
                save_samples(str(project_path(args.pred_dir)), samples, epoch, 'fixed_test1', epoch=True, cmap=args.cmap)
                samples = torch.cat((fixed_test_noisy[4:8].cpu(), 
                    fixed_denoised[4:8].cpu(), fixed_test_clean[4:8],
                    fixed_denoised[4:8].cpu() - fixed_test_clean[4:8]))
                save_samples(str(project_path(args.pred_dir)), samples, epoch, 'fixed_test2', epoch=True, cmap=args.cmap)
    ...
    save_stats(str(project_path(args.train_dir)), logger, x_axis, 'psnr_train', 'psnr_test',
