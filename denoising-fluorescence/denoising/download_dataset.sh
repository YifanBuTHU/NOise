...
TARGET_DIR=$(PYTHONPATH="$SCRIPT_DIR" python -c "from paths import DEFAULT_DATA_ROOT; print(DEFAULT_DATA_ROOT.replace('\\\\', '/'))")
...
if [ -z "$DATASET" ]; then
		folders=(Confocal_BPAE_B Confocal_BPAE_G Confocal_BPAE_R Confocal_FISH Confocal_MICE 
   		...
   		case $DATASET in
   			"confocal"|"Confocal" )
				folders=(Confocal_BPAE_B Confocal_BPAE_G Confocal_BPAE_R Confocal_FISH Confocal_MICE)
				if [ ! -d "$TARGET_DIR/test_mixed" ]; then
